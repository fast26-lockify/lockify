...
int ext4_simple_reload(struct super_block *sb) // @pty
{
	struct ext4_sb_info *sbi = EXT4_SB(sb);
	struct ext4_super_block *es = sbi->s_es;
	int err;
	...
	for (i = 0; i < db_count; i++) {
		struct buffer_head *bh;
		/* rcu_read_lock();
		bh = rcu_dereference(sbi->s_group_desc)[i];
		if (bh) {
			lock_buffer(bh);
...
EXPORT_SYMBOL(ext4_simple_reload);

unsigned long ext4_ipg(struct super_block *sb) // @pty
{
	struct inode *inode;

    list_for_each_entry(inode, &sb->s_inodes, i_sb_list) {
        // 확장 파일 시스템(ext4) 인지 확인
...
EXPORT_SYMBOL(ext4_ipg);

int ext4_reload_metadata(struct super_block *sb) // @pty
{
    struct ext4_sb_info *sbi = EXT4_SB(sb);
	struct ext4_super_block *es = sbi->s_es;
	int aborted = 0;
...
EXPORT_SYMBOL(ext4_reload_metadata);

void ext4_drop_metadata(struct super_block *sb) // @pty
{
    struct ext4_sb_info *sbi = EXT4_SB(sb);
	struct ext4_super_block *es = sbi->s_es;
	int aborted = 0;
...
EXPORT_SYMBOL(ext4_drop_metadata);

int ext4_load_metadata(struct super_block *sb) // @pty
{
    struct ext4_sb_info *sbi = EXT4_SB(sb);
	struct ext4_super_block *es = sbi->s_es;
	int aborted = 0;
...
EXPORT_SYMBOL(ext4_load_metadata);
...
int ext4_sync_super(struct super_block *sb) //@pty
{
	struct buffer_head *sbh = EXT4_SB(sb)->s_sbh;
