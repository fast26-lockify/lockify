...
struct inode *ext4_iget_remote(struct super_block *sb) //@pty
{
	struct inode *inode;
	...
	list_for_each_entry(inode, &sb->s_inodes, i_sb_list) { // TODO : 나중에는 리스트에 있는 inode만
...
EXPORT_SYMBOL(ext4_iget_remote);
			  ...
			  unsigned int line)
{
	...
	if (ret < 0)
	{
		pr_err("ext4_iget_remote: inode %lu - failed to get inode location with error %ld\n", ino, ret);
		goto bad_inode;
	}
