 ...
 */
int __vfs_open(const struct path *path, struct file *file)
{
	file->f_path = *path;
 ...
 * @file: newly allocated file with f_flag initialized
 */
int vfs_open(const struct path *path, struct file *file) //@pty
{
	int check = path->dentry->d_sb->s_flags & SB_CHECK_INVALID;
	unsigned long ino = path->dentry->d_inode->i_ino;
	int ret;
	int create = file->f_flags & O_CREAT;
