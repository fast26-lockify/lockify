...
retry:
	...
	// @pty
	error = gfs2_glock_nq_init(io_gl, LM_ST_SHARED, LM_FLAG_NOTIFY | GL_EXACT | GL_NOPID,
				   &ip->i_iopen_gh);
	if (error)
		goto fail_gunlock2;
	
	// @pty
	error = gfs2_glock_nq_init(ip->i_gl, LM_ST_EXCLUSIVE, LM_FLAG_NOTIFY | GL_SKIP, &gh);
	if (error)
		goto fail_gunlock3;
