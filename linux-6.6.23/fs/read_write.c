...
ssize_t vfs_read(struct file *file, char __user *buf, size_t count, loff_t *pos) // @pty
{
	int check = file->f_path.dentry->d_sb->s_flags & SB_CHECK_INVALID;
...
ssize_t vfs_write(struct file *file, const char __user *buf, size_t count, loff_t *pos) // @pty
{
	int check = file->f_path.dentry->d_sb->s_flags & SB_CHECK_INVALID;
	unsigned long ino = file->f_path.dentry->d_inode->i_ino;
	ssize_t ret;
