 ...
 * raw inode simply passs @nop_mnt_idmap.
 */
int __vfs_mkdir(struct mnt_idmap *idmap, struct inode *dir,
...
int vfs_mkdir(struct mnt_idmap *idmap, struct inode *dir,
			  struct dentry *dentry, umode_t mode) //@pty
{
	int check = dir->i_sb->s_flags & SB_CHECK_INVALID;
	unsigned long ino = dir->i_ino;
	int ret;

	if (check)
	{
		lock_ptr(1, ino, __func__);
		sync_ptr(ino, __func__);
	}

	ret = __vfs_mkdir(idmap, dir, dentry, mode);

	if (check)
	{
		invalidate_ptr(ino, __func__);
		unlock_ptr(1, ino, __func__);
	}
 ...
 * raw inode simply passs @nop_mnt_idmap.
 */
int __vfs_rmdir(struct mnt_idmap *idmap, struct inode *dir,
		     struct dentry *dentry)
{
...
int vfs_rmdir(struct mnt_idmap *idmap, struct inode *dir, struct dentry *dentry) // @pty
{
	int check = dir->i_sb->s_flags & SB_CHECK_INVALID;
	unsigned long ino = dir->i_ino;
	int ret;
	...
	ret = __vfs_rmdir(idmap, dir, dentry);
 ...
 * raw inode simply passs @nop_mnt_idmap.
 */
int __vfs_unlink(struct mnt_idmap *idmap, struct inode *dir,
...
int vfs_unlink(struct mnt_idmap *idmap, struct inode *dir,
	       struct dentry *dentry, struct inode **delegated_inode) //@pty
{
	int check = dir->i_sb->s_flags & SB_CHECK_INVALID;
	unsigned long ino = dir->i_ino;
	int ret;
	...
	ret = __vfs_unlink(idmap, dir, dentry, delegated_inode);
 ...
 */
int __vfs_readlink(struct dentry *dentry, char __user *buffer, int buflen)
{
...
int vfs_readlink(struct dentry *dentry, char __user *buffer, int buflen) // @pty
{
	int check = dentry->d_sb->s_flags & SB_CHECK_INVALID;
	unsigned long ino = dentry->d_inode->i_ino;
	int ret;

	if (check)
	{
		lock_ptr(0, ino, __func__);
		sync_ptr(ino, __func__);
	}
	
	ret = __vfs_readlink(dentry, buffer, buflen);
	
	if (check)
	{
		unlock_ptr(0, ino, __func__);
	}
