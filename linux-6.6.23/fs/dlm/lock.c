...
// @pty
static int send_notify(struct dlm_rsb *r, struct dlm_lkb *lkb);
...
static int set_master(struct dlm_rsb *r, struct dlm_lkb *lkb)
{
	...
	// @pty
	if (r->res_master_nodeid == our_nodeid && r->res_nodeid == -1) {
		r->res_nodeid = 0;
		lkb->lkb_nodeid = 0;
		send_notify(r, lkb);
		return 0;
...
static int do_request(struct dlm_rsb *r, struct dlm_lkb *lkb)
{
	...
	if (can_be_granted(r, lkb, 1, 0, NULL)) {
		...
		pr_info("\t\t\t\t} took %lld ns (can_be_granted)\n", duration_ns); */
		goto out;
	}
                       ...
                       struct dlm_args *args)
{
	...
	// @pty
	if (args->flags & DLM_LKF_NOTIFY) 
		error = find_rsb(ls, name, len, 0, R_REQUEST | R_NOTIFY, &r);
	else
		      ...
		      struct dlm_message *ms)
{
	...
	// @pty
	case cpu_to_le32(DLM_MSG_NOTIFY):
...
// @pty
int resend_notify(struct dlm_lkb *lkb)
{
	struct dlm_message *ms;
	struct dlm_mhandle *mh;
	...
	return send_message(mh, ms, r->res_name, r->res_length);
}
...
// @pty
int dlm_master_notify(struct dlm_ls *ls, int from_nodeid, const char *name,
                     int len, int *r_nodeid)
{
    struct dlm_rsb *r = NULL;
    uint32_t hash, b;
    int our_nodeid = dlm_our_nodeid();
    ...
    if (from_nodeid == our_nodeid) {
        log_error(ls, "dlm_master_notify from our_nodeid %d",
                 our_nodeid);
        return -EINVAL;
    }
    ...
    dir_nodeid = dlm_hash2nodeid(ls, hash);
    if (dir_nodeid != our_nodeid) {
        log_error(ls, "dlm_master_notify from %d dir %d our %d h %x %d",
                 from_nodeid, dir_nodeid, our_nodeid, hash,
                 ls->ls_num_nodes);
        *r_nodeid = -1;
        return -EINVAL;
...
// @pty
static int send_notify_reply(struct dlm_ls *ls,
			     const struct dlm_message *ms_in, int ret_nodeid,
			     int rv)
{
	struct dlm_rsb *r = &ls->ls_local_rsb;
	...
	error = create_message(r, NULL, nodeid, DLM_MSG_NOTIFY_REPLY, &ms, &mh,
			       GFP_NOFS);
	if (error)
		goto out;
...
// @pty
static void receive_notify(struct dlm_ls *ls, const struct dlm_message *ms)
{
	int len, error, ret_nodeid, from_nodeid, our_nodeid;
...
// @pty
static void receive_notify_reply(struct dlm_ls *ls,
				 const struct dlm_message *ms)
{
	struct dlm_lkb *lkb;
	int error;
			     ...
			     uint32_t saved_seq)
{
	...
	// @pty
	case cpu_to_le32(DLM_MSG_NOTIFY):
		receive_notify(ls, ms);
		break;

	case cpu_to_le32(DLM_MSG_NOTIFY_REPLY):
		receive_notify_reply(ls, ms);
		break;
